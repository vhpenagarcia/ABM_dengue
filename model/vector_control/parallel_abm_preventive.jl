...
@everywhere begin
    ...
    vectors = CSV.read("vector_pop_20.csv", DataFrame)
...
infections_def = @distributed (append!) for a in replicates
    ...
    new_inf = population2[population2.inf .== 1, :subj_cod]
    new_m_inf = vectors_2[vectors_2.inf .> 0, :code]
    for i in days
        if i == 1
            for p in 1:nrow(population2)
                population2[p,:latent] = 0
                population2[p,:inf] = 0
                population2[p,:susc] = 1
            end
            for r in 1:nrow(vectors_2)
                if vectors_2.Container_s[r] > 0 && vectors_2.type[r] == "house"
                    vectors_2[r,:Container_s] = 0
                    new_liters = if vectors_2.Container_ml[r] > 0
                        10
                    else
                        0
                    end
                    vectors_2[r,:liters] = new_liters
                end
                if vectors_2.inf[r] > 0
                    infected = vectors_2.inf[r]
                    vectors_2.inf[r] = 0
                    vectors_2.susc[r] += infected
                    vectors_2.n_vectors[r] += infected
                end
            end
        end
        update_humans(population2)
        if i == 14
            population2[in(new_inf).(population2.subj_cod),:inf] .= 1
            for r in 1:nrow(vectors_2)
                if (vectors_2.n_vectors[r] > 0) && (vectors_2.code[r] in new_m_inf)
                    vectors_2[r,:inf] = 1
                end                
            end
        end
...
CSV.write("infections_out.csv",infections_def)
