    ...
    for i in days
        if i == 50  ## Changeable
            for r in 1:nrow(vectors_2)
                if vectors_2.Container_s[r] > 0
                    number_s = rand(Binomial(vectors_2.Container_s[r],0.25)) ## Changeable
                    vectors_2[r,:Container_s] = number_s
                    new_liters = if vectors_2.Container_ml[r] > 0
                        10
                    elseif number_s > 0
                        sum(rand(truncated(Poisson(1),lower=1),number_s))
                    else
                        0
                    end
