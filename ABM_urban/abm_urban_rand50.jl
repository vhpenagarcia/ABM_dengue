        ...
        for c in 1:length(cities)
            ...
            for r in 1:nrow(vectors2)  ## Updating population of mosquitoes
                if vectors2.n_vectors[r] > 0
                    ...
                    migrants = migration(Nh,vectors2.hours[r],temp,vectors2.n_vectors[r],vectors2.liters[r],0.1) ### What level of mosquito migration? default=0.1
                    if migrants > 0
            ...
            for s in 1:nrow(vectors2) ## Core of loop, where interactions and infections are taking place
                ...
                if struc.type == "house" || struc.type == "school" || struc.type == "workplace" ## if is in a house, school or workplace
                    people = if struc.type == "house" 
                        ...
                        people_n = population2[population2.id_place .== cod_s,:subj_cod]
                        sample(people_n,round(Integer,(length(people_n)*0.5)), replace = false) ### Change to 0.5 or 0.2 for level of human movement
                    end
                    inf_h = if struc.type == "house" 
                        sum(population2[population2.house .== cod_s,:inf])
                    else
                        sum(population2[in(people).(population2.subj_cod),:inf])
                    end
                    ...
                    r_occup = if struc.type == "social"
                        human_nh(population2,vectors2,struc,cities[c],1.0,50) ### defines number of people moving
                    elseif struc.type == "market"
                        sample(population2[population2.market .== struc.code,:subj_cod],rand(DiscreteUniform(5,35)))
                    elseif struc.type == "church"
                        sample(population2[population2.church .== struc.code,:subj_cod],rand(DiscreteUniform(5,35)))
                    end
...
CSV.write("urban_rand_1k50.csv",infections_def)